...
class Document:
    ...
    def __init__(self, document_code: str, document_type: str, document_number: int, issuer: str, names: Names, country: Country, nationality: Country, dob: DocumentDob, sex: Country, check_digit: CheckDigit, expiry: DocumentDob, personal_number: str, match_document_sides: bool, address: List[str], optionals: List[OptionalElement], extra: Extra, validations: Validations) -> None:
    ...
    def from_dict(obj: Any) -> 'Document':
        ...
        return Document(document_code, document_type, document_number, issuer, names, country, nationality, dob, sex, check_digit, expiry, personal_number, match_document_sides, address, optionals, extra, validations)
...
class Match:
    ...
    def __init__(self, similarity: float, status: bool) -> None:
    ...
    def from_dict(obj: Any) -> 'Match':
        assert isinstance(obj, dict)
        similarity = from_float(obj.get("similarity"))
        status = from_bool(obj.get("status"))
        return Match(similarity, status)
...
class Face:
    ...
    def __init__(self, liveness: Liveness, match: Match) -> None:
    ...
    def from_dict(obj: Any) -> 'Face':
        assert isinstance(obj, dict)
        liveness = Liveness.from_dict(obj.get("liveness"))
        match = Match.from_dict(obj.get("match"))
        return Face(liveness, match)

    def to_dict(self) -> dict:
        result: dict = {}
        result["liveness"] = to_class(Liveness, self.liveness)
        result["match"] = to_class(Match, self.match)
        return result
...
class Payload:
    ...
    def __init__(self, id: str, company: str, face: Face, document: Document, email: Email) -> None:
        self.id = id
    ...
    def from_dict(obj: Any) -> 'Payload':
        ...
        return Payload(id, company, face, document, email)
...
class ReportID:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'ReportID':
        assert isinstance(obj, dict)
        status = from_bool(obj.get("status"))
        payload = Payload.from_dict(obj.get("payload"))
        return ReportID(status, payload)
